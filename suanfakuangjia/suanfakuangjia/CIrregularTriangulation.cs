...
namespace suanfakuangjia
{
    class CIrregularTriangulation
    {
        ...
        public int SS(CIrregularTriangulation BB, Data.Triangle[] Tri, int K, int k, ref int L, double[,] Value1, Data.Line[] Line, ref int o)
        {
            int temp, jud = 0, re = 0, k1 = 0, k2 = 0, c = 0, d = 0;
            Data.Line[] Linetemp = new Data.Line[2];
            Linetemp[0] = new Data.Line();
            Linetemp[1] = new Data.Line();
            ...
            if (temp != -1)
            {
                ...
                if (jud == 1)
                {
                    Line[o] = new Data.Line();
                    Tri[L] = new Data.Triangle();
                    Tri[L].Line[0] = new Data.Line();
                    Tri[L].Line[1] = new Data.Line();
                    Tri[L].Line[2] = new Data.Line();
        ...
        public int Judge2(Data.Triangle[] Tri, int L, Data.Line B, ref int c, ref int d)
        {           
            for (int i = 0; i < L; i++)
            {
                for (int j = 0; j < 3; j++)
